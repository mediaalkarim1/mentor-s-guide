...
export function updateMasterStore(
  ...
  if (type === "mentors") {
    if (action === "delete") {
      ...
      if (idx >= 0) {
        memoryStore.mentors[idx] = { ...memoryStore.mentors[idx], ...item };
      } else {
        memoryStore.mentors.push(item);
      }
    }
  } else if (type === "binaan") {
    if (action === "delete") {
      ...
      if (idx >= 0) {
        memoryStore.binaan[idx] = { ...memoryStore.binaan[idx], ...item };
      } else {
        memoryStore.binaan.unshift(item);
      }
    }
    ...
    if (action === "delete") {
      ...
      if (idx >= 0) {
        memoryStore.indicators[idx] = { ...memoryStore.indicators[idx], ...item };
      } else {
        memoryStore.indicators.push(item);
        memoryStore.indicators.sort((a, b) => (a.order_number || 0) - (b.order_number || 0));
      }
    }
    ...
    if (action === "delete") {
      ...
      if (idx >= 0) {
        memoryStore.periods[idx] = { ...memoryStore.periods[idx], ...item };
      } else {
        memoryStore.periods.unshift(item);
      }
      memoryStore.periods.sort((a, b) => b.start_date.localeCompare(a.start_date));
    }
  }

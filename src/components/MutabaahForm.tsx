...
export function MutabaahForm() {
  ...
  const binaanList = useMemo(
    () => (data?.binaan ?? []) as { id: string; name: string; mentor_id: string }[],
    [data?.binaan],
  );

  const mentors = useMemo(
    () => (data?.mentors ?? []) as { id: string; name: string }[],
    [data?.mentors],
  );

  const indicators = useMemo(
    () =>
      [...((data?.indicators ?? []) as any[])].sort(
  ...
  const activePeriod = data?.period ?? null;
